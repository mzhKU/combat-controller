...
24. Aug. 2025
I worked on bringing the app more into the LibGDX framework structure extending the `com.badlogic.gdx.Game` class
and `com.badlogic.gdx.Screen`.


17. Aug. 2025
![Architecture](combat-controller-making-of/20250817_architecture.jpg "Architecture")
...
1. INPUT TO COMMAND:
  User Click → InputHandler → GameCommand(playerId, action, params)

2. NETWORK TRANSMISSION:
  Client: GameCommand → JSON → WebSocket → Server
  Server: GameStateUpdate → JSON → WebSocket → Client

3. SERVER PROCESSING:
  ┌─────────────┐    ┌──────────────┐     ┌─────────────────┐
  │GameCommand  │───▶│ Validation   │───▶ │ Direct Effects  │
  │- MoveCmd    │    │- Turn check  │     │- Move entity    │
  │- FireCmd    │    │- Range check │     │- Consume ammo   │
  │- SelectCmd  │    │- Fuel check  │     │- Deal damage    │
  └─────────────┘    └──────────────┘     └─────────────────┘
                                                   │
                                                   ▼
  ┌──────────────────┐    ┌───────────────┐   ┌─────────────────┐
  │ New GameState    │◀── │ Apply Changes │◀──│ Rule Evaluation │
  │- Updated entities│    │- Fuel transfer│   │- Supply Rules   │
  │- New turn        │    │- Game over    │   │- Victory Rules  │
  │- Events log      │    │- Rule effects │   │- Auto Actions   │
  └──────────────────┘    └───────────────┘   └─────────────────┘

4. RULE ENGINE DETAIL:
  ┌─────────────────────────────────────────────────────────────┐
  │                    RULE ENGINES                             │
  │                                                             │
  │  ┌─────────────────┐  ┌─────────────────┐  ┌─────────────┐  │
  │  │  Supply Rules   │  │   Game Rules    │  │Combat Rules │  │
  │  │                 │  │                 │  │             │  │
  │  │ CannonMovesNext │  │ BaseDestroyed   │  │ Damage      │  │
  │  │ TruckMovesNext  │  │ AllUnitsLost    │  │ Range       │  │
  │  │ VehicleAtBase   │  │ TurnLimit       │  │ LineOfSight │  │
  │  └─────────────────┘  └─────────────────┘  └─────────────┘  │
  │         │                      │                    │       │
  │         ▼                      ▼                    ▼       │
  │  ┌────────────────────────────────────────────────────────┐ │
  │  │            STATE CHANGE AGGREGATOR                     │ │
...
sealed interface GameCommand {
   record Move(PlayerId player, EntityId entity, Position to) implements GameCommand
   record Fire(PlayerId player, EntityId shooter, Position target) implements GameCommand  
   record EndTurn(PlayerId player) implements GameCommand
}

// State Changes (Internal)
sealed interface StateChange {
   record EntityMoved(EntityId id, Position from, Position to) implements StateChange
   record FuelConsumed(EntityId id, int amount) implements StateChange
   record FuelTransferred(EntityId from, EntityId to, int amount) implements StateChange
   record EntityDestroyed(EntityId id) implements StateChange
   record GameOver(PlayerId winner) implements StateChange
}
...
record GameState(
   ...
   public GameState withEntities(Map<EntityId, Entity> newEntities) { ... }
...
Server → All Clients:
{
 ...
 "changes": [
   {"type": "ENTITY_MOVED", "entityId": "cannon_1", "to": {"x": 10, "y": 15}},
   {"type": "FUEL_CONSUMED", "entityId": "cannon_1", "amount": 4}
 ]
}
```
